...
import { ArrowRight, Hammer, MapPin, MessagesSquare, ShieldCheck, TriangleAlert, Users } from "lucide-react";
import { Link, useRouteLoaderData } from "react-router";
import { ForestLandscape } from "~/components/brand";
...
const STATS = [
  { value: "30,500 ha", label: "de área natural protegida" },
  { value: "1980", label: "año del decreto que protege al bosque" },
  { value: "4", label: "municipios: Zapopan, Tala, Tlajomulco y El Arenal" },
  { value: "UNESCO", label: "lo reconoce como Reserva de la Biosfera" },
];
  ...
  return (
    <>
      ...
      <section className="relative isolate overflow-hidden bg-forest-950 text-white">
        <ForestLandscape />
        <div
          className="absolute inset-0 bg-gradient-to-r from-forest-950/85 via-forest-950/45 to-transparent"
          aria-hidden
        />
        <Container className="relative flex min-h-[600px] flex-col justify-center py-20 lg:min-h-[680px]">
          <p className="inline-flex w-fit items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-sm font-medium text-forest-100 ring-1 ring-white/15">
            <MapPin className="size-4" aria-hidden /> Tala, Jalisco · Bosque La Primavera
          </p>
          <h1 className="mt-6 max-w-3xl font-display text-5xl leading-[1.05] font-semibold tracking-tight sm:text-6xl lg:text-7xl">
      ...
      <section className="bg-forest-950 pb-14">
        <Container>
          <dl className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl bg-white/10 ring-1 ring-white/10 lg:grid-cols-4">
            {STATS.map((stat) => (
              <div key={stat.label} className="bg-forest-900 px-5 py-6 sm:px-6">
                <dt className="sr-only">{stat.label}</dt>
                <dd>
                  <span className="block font-display text-3xl font-semibold text-amber-300 sm:text-4xl">
                    {stat.value}
                  </span>
                  <span className="mt-1 block text-sm text-forest-100">{stat.label}</span>
                </dd>
              </div>
            ))}
          </dl>
          <p className="mt-4 text-xs text-forest-300">
            Datos del Área de Protección de Flora y Fauna La Primavera.{" "}
            <Link to="/el-bosque" className="underline underline-offset-2 hover:text-white">
              Conoce más del bosque
            </Link>
          </p>
        </Container>
      </section>
          ...
          <ul className="grid gap-4">
            {PILLARS.map(({ icon: Icon, title, text }) => (
              <li key={title} className="flex gap-4 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-stone-200">
                <span className="flex size-12 shrink-0 items-center justify-center rounded-xl bg-forest-100 text-forest-700">
      ...
      <Section className="bg-white">
        ...
        <ul className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {PROGRAMS.map(({ icon: Icon, title, text }) => (
            <li key={title} className="rounded-2xl bg-cream p-6 ring-1 ring-earth-100">
              <Icon className="size-8 text-earth-600" aria-hidden />
              <h3 className="mt-4 font-display text-xl font-semibold text-forest-950">{title}</h3>
              <p className="mt-2 leading-relaxed text-stone-600">{text}</p>
            </li>
          ))}
        </ul>
        <div className="mt-10 text-center">
          <ButtonLink to="/que-hacemos" variant="secondary" prefetch="intent">
      ...
      {(latestPosts.length > 0 || upcomingEvents.length > 0) && (
        <Section className={activity.length > 0 ? "bg-white" : undefined}>
      ...
      <Section>
        <div className="grid gap-5 lg:grid-cols-2">
          <div className="relative overflow-hidden rounded-3xl bg-forest-800 p-8 text-white sm:p-10">
          ...
          <div className="relative overflow-hidden rounded-3xl bg-amber-100 p-8 text-forest-950 ring-1 ring-amber-200 sm:p-10">
            <TriangleAlert className="size-10 text-amber-700" aria-hidden />
            <h2 className="mt-5 font-display text-3xl font-semibold">¿Viste humo, tala o basura?</h2>
            <p className="mt-3 max-w-md text-stone-700">
            ...
            <ButtonLink to="/reportar" className="mt-8">
